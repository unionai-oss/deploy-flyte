...
resource "azuread_application_registration" "flyte-app" {
...
#  client_id                    = azuread_application_registration.flyte-app.id
...
resource "azurerm_role_assignment" "flyte_sp_role_assignment" {
  ...
  principal_id         = azuread_application_registration.flyte-app.object_id
...
resource "azuread_application_password" "flyte_client_secret" {
 # service_principal_id = azuread_service_principal.flyte_sp.object_id
  ...
  #}
  application_id = [azuread_application_registration.flyte-app.id]
}